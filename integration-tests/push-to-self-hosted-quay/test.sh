...
verify_release_contents() {
    ...
    echo "Verifying image URL points to self-hosted Quay..."
    if [[ "${image_url}" == "${self_hosted_quay_url}"/* ]]; then
        echo "✅️ image_url points to self-hosted Quay (${self_hosted_quay_url})"
    else
        echo "🔴 image_url does NOT point to self-hosted Quay. Expected prefix: ${self_hosted_quay_url}"
        failures=$((failures+1))
    fi
